...
const CATEGORIES = [
  ...
  { id: 'tech', label: 'Tehnologija', emoji: '📱', color: 'bg-slate-100 text-slate-700' },
  { id: 'health', label: 'Zdravlje', emoji: '💊', color: 'bg-red-50 text-red-700' },
  { id: 'home', label: 'Kuća', emoji: '🏠', color: 'bg-amber-50 text-amber-700' },
  { id: 'packing', label: 'Pakovanje', emoji: '🎒', color: 'bg-rose-50 text-rose-700' },
  { id: 'other', label: 'Ostalo', emoji: '✅', color: 'bg-slate-50 text-slate-600' },
]
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="font-bold text-slate-800">✅ Pre-trip checklista</h2>
          <p className="text-xs text-slate-400 mt-0.5">{done}/{tasks.length} završeno</p>
        </div>
        <button onClick={() => setShowForm(true)} className="flex items-center gap-1 text-sm bg-rose-500 text-white px-3 py-1.5 rounded-full hover:bg-rose-600 transition-colors">
      ...
      <div className="bg-white rounded-2xl p-4 shadow-sm border border-slate-100">
        ...
        <div className="h-3 bg-slate-100 rounded-full overflow-hidden">
          <div className={`h-full rounded-full transition-all ${pct === 100 ? 'bg-green-400' : 'bg-gradient-to-r from-rose-400 to-purple-500'}`} style={{ width: `${pct}%` }} />
        </div>
      ...
      <div className="flex flex-wrap gap-2 items-center">
        <div className="flex gap-1 flex-wrap">
          <button onClick={() => setFilterCat('all')} className={`px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${filterCat === 'all' ? 'bg-rose-500 text-white' : 'bg-white border border-slate-200 text-slate-600'}`}>
            Sve
          </button>
          {CATEGORIES.filter((c) => tasks.some((t) => t.category === c.id)).map((c) => (
            <button key={c.id} onClick={() => setFilterCat(c.id)} className={`px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${filterCat === c.id ? 'bg-rose-500 text-white' : 'bg-white border border-slate-200 text-slate-600'}`}>
              {c.emoji} {c.label}
            </button>
          ))}
        </div>
        <button onClick={() => setShowDone(!showDone)} className={`ml-auto px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${showDone ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-500'}`}>
      ...
      {Object.values(byCategory).map(({ id, label, emoji, color, items }) => (
        <div key={id} className="bg-white rounded-2xl p-4 shadow-sm border border-slate-100">
          ...
          <div className="space-y-2">
            {items.map((task) => (
              <div key={task.id} className={`flex items-start gap-3 p-2.5 rounded-xl ${task.done ? 'bg-green-50' : 'bg-slate-50'}`}>
                <button
                  onClick={() => togglePreTrip(trip.id, task.id)}
                  className={`w-5 h-5 rounded border-2 flex items-center justify-center flex-shrink-0 mt-0.5 transition-colors ${task.done ? 'bg-green-500 border-green-500 text-white' : 'border-slate-300 hover:border-rose-400'}`}
                >
                  {task.done && <Check size={11} />}
                </button>
                <div className="flex-1 min-w-0">
                  <div className={`text-sm font-medium ${task.done ? 'line-through text-slate-400' : 'text-slate-700'}`}>
                    {task.task}
                  </div>
                  {task.deadline && (
                  ...
                  {editId === task.id ? (
                    <div className="mt-1 flex gap-1">
                      ...
                      <button onClick={() => { updatePreTripTask(trip.id, task.id, { notes: editNotes }); setEditId(null) }} className="text-xs bg-rose-500 text-white px-2 py-1 rounded-lg">OK</button>
                    </div>
                  ) : (
                    task.notes ? (
                      <div className="text-xs text-slate-400 mt-0.5 cursor-pointer" onClick={() => { setEditId(task.id); setEditNotes(task.notes) }}>{task.notes}</div>
                    ) : (
                      <button onClick={() => { setEditId(task.id); setEditNotes('') }} className="text-xs text-slate-300 hover:text-slate-500 mt-0.5">+ napomena</button>
                    )
                  )}
                </div>
                <button onClick={() => deletePreTripTask(trip.id, task.id)} className="text-slate-300 hover:text-red-400 flex-shrink-0">
                  <Trash2 size={13} />
                </button>
              </div>
            ))}
