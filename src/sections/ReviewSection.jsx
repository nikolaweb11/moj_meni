...
export default function ReviewSection({ trip }) {
  ...
  return (
    <div className="space-y-5">
      {/* Header */}
      <div className="bg-gradient-to-r from-rose-500 to-purple-600 rounded-2xl p-5 text-white">
        <div className="text-sm font-medium text-rose-100 mb-1">Putovanje završeno</div>
        <h2 className="text-xl font-extrabold mb-1">{trip.title}</h2>
        <div className="text-rose-100 text-sm">{trip.destination} · {numDays} dana</div>
      ...
      <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100">
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-bold text-slate-800">Opšta ocena putovanja</h3>
          <button
            onClick={handleSave}
            className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-medium transition-colors ${
              saved ? 'bg-green-100 text-green-700' : 'bg-rose-500 text-white hover:bg-rose-600'
            }`}
          >
        ...
        {form.overallRating > 0 && (
          <p className="text-sm text-slate-500">{['', '😕 Loše', '😐 Ispod proseka', '🙂 Solidno', '😊 Odlično', '🤩 Savršeno putovanje!'][form.overallRating]}</p>
        )}
      </div>

      {/* Category ratings */}
      <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100">
        <h3 className="font-bold text-slate-800 mb-4">Ocene po kategorijama</h3>
        <div className="space-y-3">
          {RATING_ASPECTS.map((aspect) => (
            <div key={aspect.key} className="flex items-center gap-3">
              ...
              <span className="text-sm text-slate-600 w-36">{aspect.label}</span>
              <StarRating
                size="sm"
              ...
              {form.ratings[aspect.key] > 0 && (
                <span className="text-xs text-slate-400">{ratingLabel(form.ratings[aspect.key])}</span>
              )}
            </div>
          ))}
        </div>
        {avgRating && (
          <div className="mt-4 pt-3 border-t border-slate-100 flex items-center gap-2">
            <span className="text-sm text-slate-500">Prosek kategorija:</span>
      ...
      {/* Would go again */}
      <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100">
        <h3 className="font-bold text-slate-800 mb-3">Da li biste ponovo?</h3>
          ...
          {WOULD_GO_AGAIN.map((opt) => (
            <button
              ...
              className={`flex-1 flex flex-col items-center gap-1 py-3 rounded-xl border-2 text-sm font-medium transition-all ${
                form.wouldGoAgain === opt.id ? opt.color + ' border-current' : 'bg-slate-50 text-slate-500 border-transparent hover:border-slate-200'
              }`}
            >
      ...
      <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100 space-y-4">
        <h3 className="font-bold text-slate-800">Vaši utisci</h3>
        <div>
          <label className="text-xs text-slate-500 mb-1.5 block font-medium">📝 Kratki rezime putovanja</label>
          <textarea
            value={form.summary}
        ...
        <div>
          <label className="text-xs text-slate-500 mb-1.5 block font-medium">🌟 Najbolji momenat putovanja</label>
          <textarea
        ...
        <div>
          <label className="text-xs text-slate-500 mb-1.5 block font-medium">🔄 Šta biste uradili drugačije</label>
          <textarea
      ...
      <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="font-bold text-slate-800">📸 Foto galerija odmora</h3>
            <p className="text-xs text-slate-400 mt-0.5">{photos.length} fotografija</p>
          </div>
          <button
          ...
          <div className="bg-purple-50 rounded-xl p-4 mb-4 space-y-3">
            ...
            <div>
              ...
              >
                ...
                <p className="text-xs text-slate-500">Klikni da odabereš sliku</p>
                <p className="text-xs text-slate-400">JPG, PNG — max 2MB (automatski se kompresuje)</p>
              </div>
            ...
            <div>
              ...
              <div className="flex gap-2 mt-2">
                ...
                <button onClick={() => setShowPhotoForm(false)} className="px-4 border border-slate-200 text-slate-600 text-sm py-2 rounded-xl hover:bg-slate-50">
                  Otkaži
                </button>
              </div>
            </div>
        ...
        {photos.length === 0 ? (
          <div className="text-center py-10 bg-slate-50 rounded-xl border-2 border-dashed border-slate-200">
            ...
            <p className="text-slate-400 text-sm">Nema fotografija</p>
            <p className="text-xs text-slate-300 mt-1">Dodaj slike sa odmora da napraviš galeriju</p>
          </div>
        ) : (
            ...
            {photos.map((photo) => (
              <div key={photo.id} className="relative group rounded-xl overflow-hidden bg-slate-100 aspect-square">
                <img
                  ...
                  onError={(e) => {
                    e.target.parentElement.innerHTML = '<div class="w-full h-full flex items-center justify-center text-slate-400 text-xs p-2 text-center">Slika nije dostupna</div>'
                  }}
                />
                {photo.caption && (
        ...
        <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl p-5 text-white">
          <div className="flex items-start justify-between mb-3">
            <div>
              ...
              <p className="text-slate-400 text-sm">{trip.destination} · {format(parseISO(trip.startDate), 'dd.MM.yyyy')} — {format(parseISO(trip.endDate), 'dd.MM.yyyy')}</p>
            </div>
            {form.overallRating > 0 && (
          ...
          {form.summary && <p className="text-slate-300 text-sm leading-relaxed italic">"{form.summary}"</p>}
          {form.wouldGoAgain && (
            <div className="mt-3 pt-3 border-t border-slate-700">
              <span className="text-sm text-slate-400">Da li biste ponovo: </span>
          ...
          <div className="mt-3 text-xs text-slate-500 text-right">{couple.name1} & {couple.name2}</div>
        </div>
      )}
    </div>
  )
...
function MiniStat({ value, label }) {
  return (
    <div className="bg-white/15 rounded-xl p-2.5 text-center">
      ...
      <div className="text-rose-200 text-xs">{label}</div>
    </div>
  )
}
