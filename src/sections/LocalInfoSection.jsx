  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="font-bold text-slate-800">🗺️ Lokalne informacije</h2>
        <button onClick={save} className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-medium transition-colors ${saved ? 'bg-green-100 text-green-700' : 'bg-rose-500 text-white hover:bg-rose-600'}`}>
      ...
      <div className="bg-white rounded-2xl p-4 shadow-sm border border-slate-100 space-y-3">
        <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider">💱 Valuta & osnovne info</p>
        <div className="grid grid-cols-3 gap-2">
          <div>
            <label className="text-xs text-slate-500 mb-1 block">Lokalna valuta</label>
            <input type="text" value={form.currency} onChange={(e) => setForm((f) => ({ ...f, currency: e.target.value }))} placeholder="npr. EUR, HUF" className={inp} />
          </div>
          <div>
            <label className="text-xs text-slate-500 mb-1 block">Kurs (1 {form.homeCurrency} =)</label>
          ...
          <div>
            <label className="text-xs text-slate-500 mb-1 block">Moja valuta</label>
        ...
        <div className="grid grid-cols-3 gap-2">
          <div>
            <label className="text-xs text-slate-500 mb-1 block">Vremenska zona</label>
          ...
          <div>
            <label className="text-xs text-slate-500 mb-1 block">Napon struje</label>
          ...
          <div>
            <label className="text-xs text-slate-500 mb-1 block">Jezik</label>
      ...
      <div className="bg-white rounded-2xl p-4 shadow-sm border border-slate-100">
        <label className="text-xs text-slate-500 mb-2 block font-semibold">📱 SIM kartica / Roaming</label>
      ...
      {/* Cultural tips */}
      <div className="bg-white rounded-2xl p-4 shadow-sm border border-slate-100">
        <label className="text-xs text-slate-500 mb-2 block font-semibold">🎭 Kulturni bonton & saveti</label>
      ...
      <div className="bg-white rounded-2xl p-4 shadow-sm border border-slate-100">
        <div className="flex items-center justify-between mb-3">
          <p className="text-xs font-semibold text-slate-600 uppercase tracking-wider">💬 Korisne fraze</p>
          <button onClick={() => setShowPhraseForm(true)} className="flex items-center gap-1 text-xs text-rose-500 hover:text-rose-700">
        ...
        {showPhraseForm && (
          <div className="bg-rose-50 rounded-xl p-3 mb-3 space-y-2">
            ...
            <div className="flex gap-2">
              <button onClick={handleAddPhrase} className="flex-1 bg-rose-500 text-white text-xs py-2 rounded-lg font-medium">Dodaj</button>
              <button onClick={() => setShowPhraseForm(false)} className="px-3 border border-slate-200 text-slate-500 text-xs py-2 rounded-lg">Otkaži</button>
            </div>
          </div>
        )}

        {phrases.length === 0 ? (
          <p className="text-slate-400 text-sm text-center py-4">Nema sačuvanih fraza</p>
        ) : (
          <div className="space-y-2">
            {phrases.map((p) => (
              <div key={p.id} className="flex items-center gap-3 bg-slate-50 rounded-xl p-3">
                <div className="flex-1">
                  <div className="font-medium text-slate-800 text-sm">{p.phrase}</div>
                  {p.translation && <div className="text-xs text-slate-500">{p.translation}</div>}
                  {p.pronunciation && <div className="text-xs text-purple-500 italic">[{p.pronunciation}]</div>}
                </div>
                <button onClick={() => deletePhrase(trip.id, p.id)} className="text-slate-300 hover:text-red-400"><Trash2 size={13} /></button>
              </div>
            ))}
          </div>
        )}
